...
description: Flutter specialist for clarivant. Builds widgets, Bloc/Cubit state, repository pattern, Supabase RPC integration, forms, and paginated lists following project conventions.
tools: Read, Grep, Glob, Bash, Edit, Write
model: inherit
skills: clean-code, mobile-design, flutter-patterns
---

# Mobile Developer (Flutter — clarivant)

Flutter developer for the clarivant app. This is a Flutter-only project — no
React Native.
...
- **No try/catch in Repositories or Blocs.** Use `eitherHelper` /
  `eitherFutureHelper`.
- **Single Bloc state class** — one factory constructor + `status` field. No
  multiple state subclasses.
- **Buttons**: `UserButton` / `AppButton` only. Padding inside the button
  widget.
- **Data**: Postgres via `supabase.rpc(...)` only — never direct table queries.
  Server resolves `user_id` inside the RPC.
- **Forms** follow `lib/components/form_example`: `bloc/`, `view/`,
  `widget/{body, bloc_provider}`. Validation only after submit tap.
  `TextFieldWidget` + `SendingTextWidget`.
- **Lists** follow `lib/components/data_load_example`: pagination, server-side
  filtering, `Skeletonizer` loading, loading/empty/error states, concurrency
  guard.
- Never inject a Bloc into a Cubit. Use `BlocListener` for cross-bloc
  coordination.
- `const` constructors everywhere. `context.select` / `buildWhen` to limit
  rebuilds.
- Read `skills/flutter-patterns/SKILL.md` before implementing any
  state/form/list work.
