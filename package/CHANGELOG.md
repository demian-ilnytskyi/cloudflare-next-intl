...
- New `layoutQueriesCheck` plugin (on by default; also exposed as `cloudflare-next-intl/checkLayoutQueries` and the `cfni-check-layout-queries` CLI bin) — scans each `layout.tsx`/`layout.ts` and its reachable server-component import tree for blocking `withUserDb()` / `withPublicDb()` calls. Layouts re-render on every route transition within their group, so a DB query anywhere in that tree blocks every page switch under it; the check stops at Client Components (`'use client'`) since those don't block server layout streaming. Prints a visible terminal warning with concrete fix suggestions by default; pass `{ strict: true }` to fail the build instead.
- `vinextRouteWiringFixPlugin` gained a `refreshDeferral` sub-patch (on by default) for `app-browser-navigation-controller.js`: a `refresh()` navigation (Server Action revalidate, router refresh) landing while a normal page-to-page navigation is still in flight used to cancel that navigation outright, since both share the same `beginNavigation()` counter — the old page stayed on screen and the target route's `loading.tsx` never painted. The fix tags each `beginNavigation` call with its kind and makes a `refresh()` wait (bounded, polling every 50ms) while a non-refresh navigation is still inside a short window, instead of stepping on it.
- Added `unblockPageElementDependencies` (off by default) as a separate flag from `unblockRenderDependencies` — splits out the two sub-patches that strip `pageDependencies` from the PAGE element itself (`setResultDependencies` / the `renderAfterAppDependencies` fallback), since the configuration measured to keep page-to-page switching instant in a real app keeps vinext's own page-element wiring.
- Every vinext sub-patch (`routeWiring`, `routeMatching`, `optimisticRouting`, `prefetchLearning`, `suspenseProbe`) now checks `hasRequiredSymbols()` before touching a file: each replacement splices in references to vinext-internal helpers (`makeThenableParams`, `trieMatch`, `getPrefetchCache`, …), and if a future vinext release renames or drops one of them, a matching-but-now-invalid replacement could inject a call to an identifier that no longer exists. The patch is now skipped entirely (with a `console.warn` naming the missing symbols) whenever a required symbol is gone, so an unpatched vinext stays the fallback instead of a half-patched one crashing at runtime.
- `bustVinextOptimizeDepsCache` now also fires when `isVinextOptimizeDepsCacheStale()` finds a patched vinext file newer than Vite's `deps`/`deps_ssr`/`deps_rsc` optimizer cache — previously the cache was only busted on the same run a patch was written, so a patch applied on an *earlier* run could leave a pre-patch pre-bundle in place indefinitely, with the runtime silently executing unpatched code despite the disk file being fixed.

### Changed

- **`experimentalRouteLoadingFixes` now defaults to `true`** (was `false`), and `vinextRouteWiringFix` now follows it instead of defaulting off on its own. Every sub-patch degrades to a no-op (rather than breaking) when the vinext code it targets no longer matches, and `autoDynamicPages`'s `includeLoading` still self-disables unless the route-wiring patch verified on disk — so a future vinext release is expected to degrade to plain unpatched behavior, not break. Pass `experimentalRouteLoadingFixes: false` (or `vinextRouteWiringFix: false`) to opt back out.
- Also disabled Firebase Performance's own automatic instrumentation (`getPerformance(app)` → `initializePerformance(app, { instrumentationEnabled: false })`) since `AutoFirebasePerformanceEvents` already reports the same Web Vitals metrics itself. Two independent `PerformanceObserver` sets over the same LCP/CLS/INP entries could race, with one instance reading an entry the other had already cleared, crashing inside `web-vitals`' own internals (`Cannot read properties of undefined (reading 'startTime')`). Trade-off: this also disables Firebase's automatic network-request traces, bundled under the same flag.
- Reordered/merged the `vinextRouteWiringFixPlugin`'s Vite `transform` hook: `app-browser-entry.js` previously matched two separate `if` branches (`prefetchLearning` then a since-unreachable `optimisticLearningTimeout` branch after it), so `optimisticLearningTimeout` never actually ran through `transform` in dev. The two are now one merged branch, applied in the correct order (`prefetchLearning`'s own 3000ms insertion runs before `optimisticLearningTimeout` rewrites it).

### Fixed

- **`optimisticLearningTimeout`'s default cap raised back to 200ms** (accepts `boolean | number` — pass a number for a custom cap). This value was verified against a real app's `node_modules` to be the one that keeps page-to-page navigation instant; cutting it further makes the in-flight prefetch of the navigation target lose its race against the cap, so no optimistic template gets committed and the previous page stays on screen until the full response lands — the exact "stale loading" symptom these patches exist to fix.
- Removed the `/g` flag from `ROUTE_LOADING_GUARD_RE`: a global regex's `.test()` mutates the shared module-level `lastIndex`, so two `.test()` calls against the same string (once in the "already fixed?" check, once inside the patch) could alternate `true`/`false` and silently skip a genuine match if the guard pattern were ever duplicated in a file.
- `patchRouteMatching`'s two sub-patches (`matchRouteWithTrie`, `matchRouteWithTrieRawPathname`) no longer gate on a shared whole-file `"hasLeadingLocaleParam"` marker — patching the first function used to make that marker true and silently skip the second if both needed patching in the same run. Each is now gated on its own still-unpatched regex.
- `patchAppPageProbe`'s Suspense-visit replacement no longer unconditionally references `REACT_SUSPENSE_TYPE`; it now only does so once the declaration sub-patch actually landed (either this run or an earlier one), avoiding a spliced-in `ReferenceError` if a future vinext no longer matches the declaration's exact shape while still matching the visit-pattern shape.

### Testing

- Package coverage is enforced project-wide at 100% (statements/branches/functions/lines) per file, with a short, documented list of per-file exceptions for provably unreachable/untestable code (see `vitest.config.ts`); `vinext_route_wiring_fix.ts` joins that list this release (a tool-level function-declaration-line coverage quirk under this project's esbuild+v8 pipeline, three warn branches and their matching `transform`-hook no-op paths that are structurally unreachable given three single-regex patches' 1:1 "already fixed?" / "is this a no-op?" coupling, and two `catch` blocks guarding an unsplittable `existsSync`-then-`statSync` TOCTOU race).
