 ...
 */
const STATEMENT_CACHE = new Map<string, ParsedStatement>();
...
export default function parseStatement(sql: string): ParsedStatement {
    ...
    if (first.value === 'select') parsed = parseSelect(tokens);
    else if (first.value === 'insert') parsed = parseInsert(tokens);
    else if (first.value === 'update') parsed = parseUpdate(tokens);
    else if (first.value === 'delete') parsed = parseDelete(tokens);
    else throw new UnsupportedSqlError(`statement type "${first.value}"`);

    if (STATEMENT_CACHE.size >= MAX_STATEMENT_CACHE) STATEMENT_CACHE.clear();
