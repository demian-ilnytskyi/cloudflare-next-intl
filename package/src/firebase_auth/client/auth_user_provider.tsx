...
// session sync on it.
async function writeAppCheckTokenCookie(appCheckTokenCookieName: string, maxAge: number): Promise<boolean> {
    try {
...
class SessionSyncError extends Error {
    ...
    readonly appCheckUnavailable: boolean;

    constructor(cause: unknown, appCheckUnavailable: boolean) {
        ...
        getFirebaseAuthClient().then(async ({ auth }) => {
            ...
            unsubscribe = onIdTokenChanged(auth, async (user) => {
                ...
                try {
                    ...
                    if (e instanceof SessionSyncError && e.appCheckUnavailable) appCheckBlocked.current = true;
