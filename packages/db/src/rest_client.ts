 ...
 *
 * Memoized for the lifetime of the returned thunk, and no longer than that.
 * The main package memoized this per request with React's `cache()`; this
 * package has no request scope to hang that on, and a module-level cache
 * would outlive the request — pinning a user's JWT, and the client built
 * around it, in memory across requests. So each `createRestClient(...)`
 * result stays idempotent (one client per call site, however many statements
 * it runs) and nothing is shared beyond it.
 *
 ...
 */
async function buildClient(supabase: SupabaseDbConfig, bearerToken: string): Promise<RestClient> {
...
export default function createRestClient(supabase: SupabaseDbConfig, bearerToken: string): () => Promise<RestClient> {
    ...
    return () => (clientPromise ??= buildClient(supabase, bearerToken));
}
