...
export async function makeBlurDataURL(primaryBuffer, targetFile, sourceWidth, sourceHeight, blurOptions) {
...
async function encodeFormat(targetFile, source, sourcePath, format, quality, effort, targetWidth) {
...
async function processVariant(absolutePath, sourceBuffer, publicSrc, targetFile, targetSrc, targetWidth, sourceWidth, sourceHeight, config, isDefault) {
    ...
    const [primaryBuffer] = await Promise.all([
        ...
        ...siblingTargets.map((target) => encodeFormat(target.file, sourceBuffer, absolutePath, target.format, config.quality, config.effort, targetWidth)),
    ]);
    const sources = [
...
export async function processImage(absolutePath, publicRoot, options, root = path.dirname(publicRoot)) {
    ...
    const defaultVariant = await processVariant(absolutePath, sourceBuffer, publicSrc, targetFile, targetSrc, defaultTargetWidth, sourceWidth, sourceHeight, config, true);
    const variants = [
        defaultVariant,
        ...(await Promise.all(extraTargetWidths.map((width) => processVariant(absolutePath, sourceBuffer, publicSrc, targetFile, targetSrc, width, sourceWidth, sourceHeight, config, false)))),
    ];
    return {
