    ...
    test: {
        ...
        coverage: {
            ...
            thresholds: {
                ...
                // insert_locale_params.ts: insertLocaleParamsSignature's `parensMatch === null` branch is unreachable — ZERO_ARG_DEFAULT_EXPORT's own `(?=\(\s*\))` lookahead guarantees the immediately-following ZERO_ARG_PARENS match always succeeds whenever the name match does (confirmed directly: slicing the source at the name match's end and re-running ZERO_ARG_PARENS on it always matches at index 0). findParamListSpan's `openParenMatch === null` branch is equally unreachable through its one caller (wrapSyncDefaultExportWithParams): it only ever calls findParamListSpan right after SYNC_DEFAULT_EXPORT_FUNCTION has already matched, and DEFAULT_EXPORT_FUNCTION_OPEN_PAREN is that same pattern with an added optional `(?:async\s+)?` — a strict superset — so it always matches too whenever SYNC_DEFAULT_EXPORT_FUNCTION does.
                'src/locale_params_check/insert_locale_params.ts': { statements: 100, branches: 98.43, functions: 100, lines: 100 },
