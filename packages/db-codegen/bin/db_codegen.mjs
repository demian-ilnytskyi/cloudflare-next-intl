...
//                        [--schema-import=…]
//
// --schema-import (also CFNI_DB_SCHEMA_IMPORT) sets the module specifier
// drizzle-kit's raw `drizzle-orm`/`drizzle-orm/pg-core` imports get
// retargeted to, so the generated schema doesn't need a direct drizzle-orm
// dependency to load. Defaults to `cloudflare-next-intl-db/schema`; pass
// `--schema-import=cloudflare-next-intl/dbSchema` when generating for the
// legacy `cloudflare-next-intl` package instead.
//
...
import resolveCodegenPaths from '../dist/src/codegen_paths.js';
...
// Known drizzle-kit 0.31.10 introspection bug: a `text`/`varchar` column
// whose Postgres default is the empty string (`DEFAULT ''`) is emitted as
// `.default(')` — a truncated, unterminated string literal that throws a
// syntax error on import — instead of `.default('')`. Root cause (confirmed
// in node_modules/drizzle-kit/utils.js's `unescapeSingleQuotes`): the raw
// default text pulled from Postgres for an empty string is the 2-character
// literal `''`. `unescapeSingleQuotes(str, true)` is meant to strip the
// outer quote pair while unescaping any inner `''` pair, but its
// `ignoreFirstAndLastChar` regex (`/(?<!^)'(?!$)/g`) first runs
// `str.replace(/''/g, "'")` unconditionally, collapsing the 2-char string to
// a single orphan `'` *before* the position-aware regex ever sees a first or
// last character to exempt — so the single surviving quote gets escaped
// into `\'` and reinjected into the generated file as an unterminated
// string. Every other default shape (non-empty text, numeric, `sql\`\``
// expressions) is unaffected; only the empty-string case collapses to zero
// content first. There is no newer drizzle-kit release (0.31.10 is already
// `latest`) and no pull/config flag that changes this. This patch detects
// the resulting `.default(')` shape and rewrites it back to the correct
// `.default('')`.
function patchEmptyStringDefaults(source) {
...
function retargetDrizzleImports(source, schemaImport) {
    ...
    const names = [...(coreMatch ? [coreMatch[1]] : []), ...(rootMatch ? [rootMatch[1]] : [])]
        ...
        .map((name) => name.trim())
        .filter(Boolean);
    const merged = `import { ${[...new Set(names)].join(", ")} } from "${schemaImport}"`;
...
const pulledSource = retargetDrizzleImports(
    patchEmptyStringDefaults(patchBareFunctionCallDefaults(readFileSync(pulled, "utf8"))),
    paths.schemaImport,
);
rmSync(paths.pullDir, { recursive: true, force: true });
for (const target of paths.targets) {
